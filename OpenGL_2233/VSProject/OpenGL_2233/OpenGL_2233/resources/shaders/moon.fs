...
{    
    ...
    float light = max(dot(-lightDirection, Normals), 0.0); //set de edge iets meer naar achter met +.25
    light = pow(light * 128.0, 2.0) / 128.0; //16 is de edge waarden van de light/dark planet edge
